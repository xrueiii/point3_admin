    ...
    <>
      <div className="lg:w-3/4 w-full flex gap-4 px-8 lg:px-32 py-2 mx-auto h-20">
        ...
        {date.map((d) => (
          <div
            key={d}
            className="flex justify-center items-center w-1/6 rounded-md bg-black text-white text-center lg:text-lg text-sm break-all"
          >
            {d}
          </div>
        ))}
          ...
          <div className="flex-cols w-1/6 space-y-4">
            {d.map((r, roomIndex) =>
              r.displayName !== "" ? (
                ...
                <div className="h-[25%] flex justify-center items-center rounded-md w-full bg-[#333] text-white">
                  暫無預約
                </div>
              )
            )}
